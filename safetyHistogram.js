(function (global, factory) {
    ...
    function syncSettings(settings) {
      ...
      settings.group_by = settings.group_by && settings.groups.some(function (group) {
        return group.value_col === settings.group_by;
      }) ? settings.group_by : settings.groups.length > 1 ? settings.groups[1].value_col : defaultGroup.value_col;
    ...
    function measure() {
      ...
      this.measures = this.initial_data[0].hasOwnProperty(this.config.measure_order_col) ? _toConsumableArray(new Set(this.initial_data.map(function (d) {
      ...
      }) : _toConsumableArray(new Set(this.initial_data.map(function (d) {
        return d[_this.config.measure_col];
      })).values()).sort(webcharts.dataOps.naturalSorter); // Define set of measure values with units (in ADaM units are already attached; in SDTM units are captured in a separate variable).

      this.sh_measures = this.initial_data[0].hasOwnProperty(this.config.measure_order_col) ? _toConsumableArray(new Set(this.initial_data.map(function (d) {
        return +d[_this.config.measure_order_col];
      })).values()).sort(function (a, b) {
        return a - b;
      }).map(function (value) {
    ...
    function identifyControls() {
      var context = this;
      var controlGroups = this.controls.wrap.selectAll('.control-group').style('vertical-align', 'middle'); // Give each control a unique ID.
    ...
    function customizeGroupByControl() {
      ...
      if (groupControl.datum().values.length === 1) groupControl.style('display', 'none');else {
        groupControl.attr('title', 'Select a variable by which to stratify the distribution on the left.');
    ...
    function updateParticipantCount() {
      ...
      this.participantCount.percentage = d3.format('0.1%')(this.participantCount.n / this.participantCount.N); // clear the annotation
    ...
    Normality.shapiroWilk = function (x) {
      ...
      var an = -2.706056 * Math.pow(u, 5) + 4.434685 * Math.pow(u, 4) - 2.07119 * Math.pow(u, 3) - 0.147981 * Math.pow(u, 2) + 0.221157 * u + c.elements[n - 1];
    ...
    function pValueFormat(p) {
      ...
      var pFmt = p < 0.001 ? '<0.001***' : p === 0.001 ? '0.001***' : p < 0.01 ? "".concat(fmt(p), "**") : p < 0.05 ? "".concat(fmt(p), "*") : p < 0.999 ? fmt(p) : p <= 1.0 ? '>0.999' : 'NA';
      return pFmt;
    }
    ...
    function runShapiroWilkTest() {
      ...
      if (this.config.test_normality) {
        ...
        pValue.append('span').classed('sh-statistical-test__p-value', true).attr('title', 'The Shapiro-Wilk test tests the null hypothesis that a sample x[1], ..., x[n] came from a normally distributed population.').text("p = ".concat(pValueFormat(this.sw.p)));
        pValue.append('span').classed('sh-statistical-test__info', true).attr('title', "".concat(validationMessage(), "\nClick to view information on the Shapiro-Wilk normality test.")).html(' &#9432').on('click', function () {
    ...
    function runKolmogorovSmirnovTest() {
      ...
      if (this.config.compare_distributions && this.config.group_by !== 'sh_none') {
        ...
        this.groups = d3.set(this.initial_data.map(function (d) {
    ...
    function annotatePValues() {
      ...
      if (this.sh.config.compare_distributions) {
        ...
        pValue.append('span').classed('sh-statistical-test__p-value', true).attr('title', 'The two-sample Kolmogorov-Smirnov test tests whether two underlying one-dimensional probability distributions differ.\n' + "In this case the test compares the distribution of results where ".concat(this.sh.config.group_label, " = ").concat(this.filters[0].val, " with that of the full set of results.")).text("p = ".concat(pValueFormat(this.ks.p)));
        pValue.append('span').classed('sh-statistical-test__info', true).attr('title', "".concat(validationMessage(), "\nClick to view information on the Kolmogorov-Smirnov two-sample test.")).html(' &#9432').on('click', function () {
      ...
      if (this.config.draw_multiples) {
        ...
        otherCharts.forEach(function (chart) {
          chart.marks[0].groups.each(function (di) {
            if (di.key === d.key) d3.select(this).selectAll('.bar').attr('stroke', context.colorScale());
          });
        });
      }
    }
    ...
    function select(element, d) {
      ...
      safetyHistogram.svg.selectAll('.bar-group').selectAll('.bar').attr('fill-opacity', 0.5); // Reduce bin opacity of all bars in small multiples.
      ...
      d3.select(element).select('.bar').attr('fill-opacity', 1); // Update bar click footnote.
    ...
    function drawNormalRanges() {
      ...
      if (this.config.display_normal_range && this.filtered_data.length > 0) {
        ...
        .rollup(function (d) {
          ...
          d.tooltip = d.values < _this.filtered_data.length ? "".concat(d.lower, " - ").concat(d.upper, " (").concat(d3.format('%')(d.rate), " of records)") : "".concat(d.lower, " - ").concat(d.upper); // plot if:
